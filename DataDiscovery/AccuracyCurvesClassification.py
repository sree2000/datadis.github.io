...
trainers=[RandomForestClassifier(),GaussianNB(),RidgeClassifier(),neighbors.KNeighborsClassifier(n_neighbors=5)]
names=["RFC","GNB","Ridge","KNC"]
models=[RandomForestClassifier(),GaussianNB(),RidgeClassifier(),neighbors.KNeighborsClassifier(n_neighbors=5)]

scoring = ['accuracy','balanced_accuracy','average_precision','neg_brier_score','f1_micro',
'f1_macro','f1_weighted','f1_samples','neg_log_loss','precision_micro','precision_macro',
'precision_weighted','precision_samples','recall_micro','recall_macro','recall_weighted','recall_samples',
'jaccard_micro',
'jaccard_macro',
'jaccard_weighted',
'jaccard_samples',
...
#   print(1-(sum(y_true)-sum(y_pred))/sum(y_true))
  # return np.mean(1-((sum(y_true)-sum(y_pred))/avg(y_true)))
